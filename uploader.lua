...
local loadBtn = mkButton(content, "LOAD MODEL", C.accent, C.bg, 28)
loadBtn.TextSize = 11
...
local function collectBricks()
    ...
    local bricks = {}
    local pivot = loadedModel:GetPivot()
    local skipped = 0
    local totalParts = 0

    -- ===== AUTO-DETECT Obby Creator part type from Roblox part shape =====
    -- Maps each Roblox BasePart to its closest Obby Creator equivalent.
    local function detectObbyType(p)
        -- WedgePart -> Wedge
        ...
        if p:IsA("Part") then
            ...
            return "Part"  -- Block shape
        end
        -- MeshPart / UnionOperation -> check for SpecialMesh child, otherwise Part
        if p:IsA("MeshPart") or p:IsA("UnionOperation") then
            -- Check for SpecialMesh child with a recognizable MeshType
            for _, child in ipairs(p:GetChildren()) do
                if child:IsA("SpecialMesh") then
                    ...
                    if mt == Enum.MeshType.FileMesh then
                        -- Could be a cone, pyramid, etc. — can't tell from MeshType alone.
                        -- Default to Part (mesh data can't be imported anyway).
                        return "Part"
                    end
                end
            end
    ...
    for _, p in ipairs(loadedModel:GetDescendants()) do
        if p:IsA("BasePart") then
            ...
            -- Build ALL parts (including transparent + non-collide)
            local obbyType = detectObbyType(p)

                -- ===== SCAN FOR EFFECTS (Fire, SurfaceLight, Texture, Decal) =====
                local effects = {}
                for _, child in ipairs(p:GetChildren()) do
                    ...
                    elseif child:IsA("PointLight") then
                        -- PointLight becomes a SurfaceLight in Obby Creator
                        table.insert(effects, {
                            type = "surfacelight",
                            props = {
                                {"Brightness", child.Brightness},
                ...
                -- ===== SPLIT PARTICLES FROM OTHER EFFECTS =====
                -- If a part has multiple ParticleEmitters, we need to clone the part
                -- (one copy per particle emitter) so each gets its own particle effect.
                -- Non-particle effects (fire, surfacelight, texture) go on the FIRST copy only.
                ...
                local function addBrick(effectsList)
                    table.insert(bricks, {
                        part = p,
                        relCFrame = pivot:ToObjectSpace(p.CFrame),
                        size = p.Size,
                        color = p.Color,
                        material = p.Material,
                ...
                if #particleEffects <= 1 then
                    -- 0 or 1 particle: single brick with all effects (normal behavior)
                    addBrick(effects)
                else
                    -- Multiple particles: clone the part for each particle emitter
                    -- First copy gets other effects + first particle
                    local firstEffects = {}
                    ...
                    -- Each subsequent copy gets only its particle effect
                    for i = 2, #particleEffects do
                        addBrick({particleEffects[i]})
                    end
                    log("Part '" .. (p.Name or "?") .. "' has " .. #particleEffects ..
                        " particle emitters — cloned into " .. #particleEffects .. " parts", C.warn)
                end
        end
    end
...
function startBuild(originCFrame)
    ...
    spawn(function()
        ...
        for i, brick in ipairs(bricks) do
            ...
            -- 2) Find the NEW part — the one that wasn't in our snapshot.
            --    Try immediately first, then wait for ChildAdded as backup.
            local newPart = nil
            for _, ch in ipairs(partsFolder:GetChildren()) do
            ...
            -- If not found immediately, wait for the next child added (up to 2 seconds)
            if not newPart then
                local conn
                local found
                conn = partsFolder.ChildAdded:Connect(function(child)
                    found = child
                end)
                local waited = 0
                while not found and waited < 2 do
                    wait(0.05)
                    waited = waited + 0.05
                end
        ...
        if totalEffects > 0 then
            ...
            for i, entry in ipairs(placedParts) do
                ...
                setProgress(0.75 + i / #placedParts * 0.25, "Phase 4/4: Effects " .. i .. "/" .. #placedParts)
                wait(buildSpeed)
            end
...
loadBtn.MouseButton1Click:Connect(function()
    ...
    -- Extract numeric ID (handles URLs like rblox.com/library/1234567890/...)
    ...
    loadStatus.Text = "Loading model " .. id .. "..."
    loadStatus.TextColor3 = C.warn
    log("Loading model ID " .. id, C.accent)
    ...
    spawn(function()
        -- ===== LOAD MODEL =====
        -- InsertService:LoadAsset is blocked in executors (client-side restriction).
        -- Use game:GetObjects("rbxassetid://ID") instead — executors support this.
        local model = nil
        local loadErr = nil

        -- Method 1: game:GetObjects (executor function — preferred)
        local ok, result = pcall(function()
            return game:GetObjects("rbxassetid://" .. id)
        end)
        if ok and type(result) == "table" and #result > 0 then
            model = result[1]
            log("Loaded via game:GetObjects", C.success)
        else
            loadErr = tostring(result)
            log("game:GetObjects failed: " .. loadErr, C.warn)

            -- Method 2: InsertService fallback (usually fails in executors, but try anyway)
            ok, result = pcall(function()
                return InsertService:LoadAsset(id)
            end)
            if ok and result then
                model = result
                -- If it's a wrapped asset, get the inner model
        ...
        if not model then
            ...
            log("Your executor may not support game:GetObjects.", C.warn)
            log("Try a different executor or model ID.", C.warn)
            return
        end

        -- ===== FIND THE RICHEST MODEL (most BaseParts) =====
        -- game:GetObjects often returns a wrapper. The actual model with all the parts
        -- might be nested several levels deep. Search for the model with the most BaseParts.
        ...
        -- Search all descendant Models/Folders for one with more parts
        for _, d in ipairs(model:GetDescendants()) do
            if d:IsA("Model") or d:IsA("Folder") or d:IsA("Tool") then
        ...
        -- Also check if the loaded object itself is a BasePart (single-part model)
        ...
        if meshParts > 0 then
            log("Note: MeshParts will be built as regular Parts (mesh data can't be imported).", C.warn)
        end

        -- Store the model (hide it)
        loadedModel = model
        model.Parent = workspace
        model:PivotTo(CFrame.new(0, -1000, 0))  -- hide far below
